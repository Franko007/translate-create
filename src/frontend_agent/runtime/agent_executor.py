"""A2A Agent Executor for the Frontend Agent."""
...
class FrontendExecutor(AgentExecutor):
    def __init__(self):
        self.agent = None
        self.runner = None
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            async for event in self.runner.run_async(
                session_id=context.task_id, user_id="frontend_user", new_message=content,
            ):
            ...
            if final and final.content and final.content.parts:
                ...
                if text:
                    await updater.add_artifact([TextPart(text=text)], name="frontend_report")
                    await updater.complete()
                    return
            await updater.update_status(TaskState.failed, message=new_agent_text_message("No frontend report"), final=True)
        except Exception as e:
            logger.exception("Frontend build failed")
            await updater.update_status(TaskState.failed, message=new_agent_text_message(f"Frontend failed: {e}"), final=True)
